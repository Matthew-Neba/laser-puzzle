...
void load_laser_puzzle_levels(LaserPuzzle* env, const char* path) {
    ...
    env->num_levels = (int)header[2];
    env->levels = (LaserPuzzleLevel*)calloc((size_t)env->num_levels, sizeof(LaserPuzzleLevel));

    for (int i = 0; i < env->num_levels; i++) {
        fread(&env->levels[i].optimal_mirrors, sizeof(int), 1, file);
        fread(&env->levels[i].sensor_count, sizeof(int), 1, file);
        for (int r = 0; r < INIT_ROWS; r++) {
            for (int c = 0; c < INIT_COLS; c++) {
                uint8_t raw[4] = {0};
                fread(raw, sizeof(raw), 1, file);
                env->levels[i].puzzle[r][c] = (Cell){
                    .type = (CellType)raw[0],
...
void c_reset(LaserPuzzle* env) {
    ...
    int level_index = rand() % env->num_levels;
